...
use std::{env::{var, VarError}, fs::{self, File}, io::Write, rc::Rc, str::FromStr, sync::Mutex, vec::Vec};
...
static PWM_DATA: Lazy<Mutex<PwmGuiData>> = Lazy::new(|| Mutex::new({
...
impl PwmSettingsAccess for PwmSettings {
    ...
    fn add_setting(&mut self, name: String) {
        self.settings.push(once_cell::sync::Lazy::<PwmSetting>::force(&PWM_DEFAULT).clone());
    ...
    fn delete_setting(&mut self) {
        if self.settings.is_empty() { return };
...
impl<'a> PwmGui<'a> for PwmGuiData {
    fn new() -> Self {
        PwmGuiData { settings: PwmSettings::new(),
              settings_error: PwmSettingsError::Ok,
              error: PwmConfigError::Ok }
    }

fn create_settings(&mut self) {
    self.settings.settings.push(once_cell::sync::Lazy::<PwmSetting>::force(&PWM_DEFAULT).clone());
...
fn save_settings(&mut self) -> Result<(), PwmConfigError> {
    let home = match get_home_dir() {
        Ok(home) => home,
        Err(_e) => { self.error = PwmConfigError::NoHome; return Err(self.error) }
    };
    ...
    let mut output = match File::create(path) {
        Ok(output) => output,
        Err(_e) => {self.error = PwmConfigError::FailOpenForWrite; return Err(self.error) }
    };
    match write!(output, "{}", toml) {
        Ok(_) => Ok(()),
        Err(_) => { self.error = PwmConfigError::FailWrite; Err(self.error) }
    }
}
    ...
    fn pwm_from_setting(&'a mut self) -> Result<Pwm<'a>, PwmSettingsError> {
        let setting = match self.settings.settings.get(self.settings.current_setting) {
            ...
            None => { return Err(PwmSettingsError::NoSetting); }
        };
...
fn create_use_leet_when_generating(use_leet: &str, leet_level: &str) -> Result<UseLeetWhenGenerating, LeetError>
{
    let ll = LeetLevel::from_str(leet_level);
    let uldr: Result<UseLeetWhenGeneratingDiscriminants, _> = UseLeetWhenGeneratingDiscriminants::from_str(use_leet);
    match uldr {
        Ok(u) => match u {
            UseLeetWhenGeneratingDiscriminants::NotAtAll => Ok(UseLeetWhenGenerating::NotAtAll),
            UseLeetWhenGeneratingDiscriminants::Before if ll.is_ok() => Ok(UseLeetWhenGenerating::Before { level: ll.unwrap() }),
            UseLeetWhenGeneratingDiscriminants::After if ll.is_ok() => Ok(UseLeetWhenGenerating::After { level: ll.unwrap() }),
            UseLeetWhenGeneratingDiscriminants::BeforeAndAfter if ll.is_ok() => Ok(UseLeetWhenGenerating::BeforeAndAfter { level: ll.unwrap() }),
...
{
  match PWM_DATA.lock() {
    Ok(mut pwm) => pwm.create_password(url.to_string(), master.to_string()).into(),
...
fn main() -> Result<(), PwmConfigError> {
    ...
    app.global::<UiSettings>().set_hash_algorithms(get_vecmodel_from_enum(HashAlgorithm::VARIANTS));
    app.global::<UiSettings>().set_use_leet(get_vecmodel_from_enum(UseLeetWhenGenerating::VARIANTS));
    app.global::<UiSettings>().set_leet_level(get_vecmodel_from_enum(LeetLevel::VARIANTS));
    app.global::<UiSettings>().set_current_setting(on_get_current_setting());
    app.global::<UiSettings>().set_setting(on_get_setting_data());
    app.global::<UiSettings>().set_available_settings(on_get_available_settings());
